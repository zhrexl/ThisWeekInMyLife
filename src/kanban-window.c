...
static void response(AdwMessageDialog *self, gchar *response,
  ...
  GApplication *app =
      G_APPLICATION(gtk_window_get_application(GTK_WINDOW(user_data)));
  g_application_quit(app);
}

static gboolean save_before_quit(KanbanWindow *self) {
  ...
  adw_message_dialog_format_body(
      ...
      ("Open document contains unsaved changes. Changes which are not saved "
       "will be permanently lost."));

  adw_message_dialog_add_responses(ADW_MESSAGE_DIALOG(dialog), "cancel",
                                   ("_Cancel"), "discard", ("_Discard"), "save",
                                   ("_Save & Quit"), NULL);

  adw_message_dialog_set_response_appearance(
      ADW_MESSAGE_DIALOG(dialog), "discard", ADW_RESPONSE_DESTRUCTIVE);
...
static gboolean item_drag_drop(GtkDropTarget *dest, const GValue *value,
                               double x, double y) {
  ...
  KanbanColumn *col = KANBAN_COLUMN(
      gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(dest)));
...
static void remove_column(KanbanColumn *Column, gpointer user_data) {
...
GtkWidget *create_column(KanbanWindow *Window, const gchar *title) {
  ...
  g_object_bind_property(column, "needs-saving", Window->save, "sensitive",
                         G_BINDING_BIDIRECTIONAL);
  g_object_bind_property(column, "edit-mode", Window->EditBtn, "active",
                         G_BINDING_BIDIRECTIONAL);
...
static void kanban_window_class_init(KanbanWindowClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/com/github/zhrexl/kanban/kanban-window.ui");
  ...
  gtk_widget_class_bind_template_child(widget_class, KanbanWindow,
                                       toast_overlay);

  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(klass), save_cards);
}

static void create_cards_from_json(KanbanColumn *Column, JsonNode *node) {
  ...
  for (GList *child = list; child != NULL; child = child->next) {
    ...
    const gchar *description =
        json_object_get_string_member(objmember, "description");
...
static int loadjson(KanbanWindow *self, gchar *file_path) {
...
static gboolean load_ui(KanbanWindow *self) {
...
static void kanban_window_init(KanbanWindow *self) {
