...
KanbanCard *kanban_card_new(void) {
  return g_object_new(KANBAN_TYPE_CARD, NULL);
}
...
static void create_task(GtkTextView *text_view, GtkTextIter *iter,
                        const gchar *title, gboolean active) {

  GtkTextChildAnchor *anchor = gtk_text_buffer_create_child_anchor(
      gtk_text_view_get_buffer(text_view), iter);
...
static void insert_checkbox(GtkButton *btn, gpointer data) {
...
static void delete_clicked(GtkButton *btn, gpointer user_data) {
...
static void reveal_clicked(GtkButton *btn, gpointer user_data) {
...
static void kanban_card_class_init(KanbanCardClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/com/github/zhrexl/kanban/kanban-card.ui");
...
static GdkContentProvider *css_drag_prepare(GtkDragSource *source, double x,
                                            double y, GtkWidget *button) {
...
static void kanban_card_changed(GtkTextBuffer *buf, gpointer user_data) {
  g_object_set(G_OBJECT(user_data), "needs-saving", 1, NULL);
}

static void kanban_card_title_changed(GtkEditableLabel *label,
                                      gpointer user_data) {
  g_object_set(user_data, "needs-saving", 1, NULL);
}

static void kanban_card_init(KanbanCard *self) {
  ...
  self->description_changed =
      g_signal_connect(buf, "changed", G_CALLBACK(kanban_card_changed), self);

  g_signal_connect(self->LblCardName, "changed",
                   G_CALLBACK(kanban_card_title_changed), self);
  // Both the description and the title are separate, so we had to implement
  // them independently
}
